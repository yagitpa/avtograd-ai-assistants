...
Адрес для клиентов — `http://127.0.0.1:8080`, а не `http://localhost:8080`.
Ядро слушает IPv4; Node, а с ним и n8n, разрешает `localhost` сначала в
IPv6-адрес `::1`, где никто не слушает. Windows отвечает на это WinError
10013, n8n показывает его как «Forbidden by access permissions», и час
