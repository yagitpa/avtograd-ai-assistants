...
![Отказ сообщить историю обслуживания по госномеру чужого автомобиля](../screenshots/07-third-party-refusal.png)

Госномер видно на парковке, VIN — под лобовым стеклом: ни то ни другое не подтверждает прав на автомобиль. Поэтому отказ опирается не на проверку личности, а на то, как собеседник сам себя назвал, и сопровождается выходом — вопрос уйдёт мастеру-приёмщику, либо владелец напишет сам.
...
![Внутренний бот: сотрудник спрашивает про отпуск, ассистент отвечает по регламенту и предлагает передать запрос HR](../screenshots/06-staff-internal-contour.png)

Внутренний контур отдельным ботом: другое приветствие, другая аудитория, другой круг вопросов. Ассистент отвечает по регламенту — срок подачи заявления, согласование, количество дней — и отправляет за остатком дней в портал, а не выдумывает число. Расчёты и спорные ситуации уходят HR бизнес-партнёру.
...
## Персональные данные не доходят до модели

VIN, телефон, имя и госномер заменяются плейсхолдерами **до** обращения к модели и разворачиваются обратно в ответе. Клиент видит свой VIN, модель его не видит.

![Слева чат с настоящими VIN и телефоном, справа журнал процесса с плейсхолдерами](../screenshots/05-pii-gateway.png)

Слева клиент прислал VIN и телефон и получил ответ с ними же. Справа журнал того же обращения: в модель ушло `{{VIN_1}}` и `{{PHONE_1}}`. Поиск по базе работает **до** шлюза — ему нужны настоящие номера, и он локальный; за границу процесса уходят уже плейсхолдеры.

Последняя строка журнала — `контакт 1 · диалог 12 · service · промпт e0038678`. Версия промпта пишется в каждый ответ: когда через месяц придёт жалоба, вопрос «а что тогда было написано в промпте» имеет ответ.

Включается трассировка переменной `AVTOGRAD_TRACE_PII=1` и по умолчанию молчит. В журнал попадает только обезличенный текст и виды подмен — сами значения никогда: лог с настоящими VIN и телефонами стал бы тем самым хранилищем персональных данных, от которого шлюз и защищает.
